...
hit findHit(ray r, sphere[4] s) 
{
    ...
    for(int i=0; i<4; i++) { 
        ...
        // if (h == dummyHit || (temp != dummyHit && temp.t <= smallest)) -- also worked
        if (temp.t < smallest && temp.t > minT) {
            h = temp; 
            smallest = temp.t;
        }
    }
